...
In the original latent space in which there is no noise, the condition for which community detection is possible via $\eta$-neighborhood graph clustering or single linkage clustering is satisfied if each pair of adjacent order statistics on each manifold is less than $\delta$ apart. 
...
Define $y_i = x_i + e_i$ and $B_n$ as the event such that $\|y_{(i+1)}^{(k)} - y_{(i)}^{(k)}\| < \|y_{(i)}^{(k)} - y_{(j)}^{(\ell)}\|$ for any $k \neq \ell$, where, $y_{(i)}^{(k)}$ is the $i^{th}$ order statistic on the $k^{th}$ manifold plus its corresponding noise. 
