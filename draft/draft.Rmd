...
plots.list <- lapply(seq_along(out), function(i) {
  ...
  t.1 <- seq(min(out[[i]]$t[[1]]), max(out[[i]]$t[[1]]), .01)
  t.2 <- seq(min(out[[i]]$t[[2]]), max(out[[i]]$t[[2]]), .01)
  Y1 <- bezier.curve(t.1, out[[i]]$p[[1]])
  Y2 <- bezier.curve(t.2, out[[i]]$p[[2]])
...
```{r}
t.1 <- seq(min(c(0, macaque.out$t[[1]])), max(macaque.out$t[[1]]), .01)
t.2 <- seq(min(c(0, macaque.out$t[[2]])), max(macaque.out$t[[2]]), .01)
Y1 <- bezier.curve(t.1, macaque.out$p[[1]])
Y2 <- bezier.curve(t.2, macaque.out$p[[2]])

ggplot() + 
  geom_point(aes(x = Xhat[, 1], y = Xhat[, 2], 
                 colour = factor(macaque.out$z)),
             size = .5) + 
  labs(x = expression(x[1]), y = expression(x[2]), colour = NULL) + 
  theme_bw() + 
  coord_fixed() +
...
```{r}
t.1 <- seq(min(nonintersect.out$t[[1]]), max(nonintersect.out$t[[1]]), .01)
t.2 <- seq(min(nonintersect.out$t[[2]]), max(nonintersect.out$t[[2]]), .01)
Y1 <- bezier.curve(t.1, nonintersect.out$p[[1]])
Y2 <- bezier.curve(t.2, nonintersect.out$p[[2]])

ggplot() + 
  geom_point(aes(x = Xhat[, 1], y = Xhat[, 2], 
                 colour = factor(nonintersect.out$z)),
             size = .5) + 
  labs(x = expression(x[1]), y = expression(x[2]), colour = NULL) + 
  theme_bw() + 
  coord_fixed() +
