...
$X = \begin{bmatrix} x_1 & \cdots & x_n \end{bmatrix}$ such that each $x_i \in \mathcal{M}_k$ for some $k \in [K]$, then $A \sim \GRDPG_{p,q}(X; \rho_n)$ is a \emph{manifold block model}.
\end{definition}

In practice, we define each manifold $\mathcal{M}_k$ by a function $f_k : [0, 1]^r \to \mathcal{X}$, where $1 \geq r < d$ is the dimensionality of manifold $\mathcal{M}_k$. 
...
Draw $t_1, ..., t_n \sim \Uniform(0, 1)$ and $z_1, ..., z_n \Multinomial(\frac{1}{2}, \frac{1}{2})$, and compute latent vectors $x_i = f_{z_i}(t_i)$, which are collected in data matrix $\begin{bmatrix} x_1 & \cdots & x_n \end{bmatrix}^\top$. 
